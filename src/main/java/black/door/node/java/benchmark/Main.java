	...
	public static void main(String[] args){
		new Node()
				.get("^/fib/(?<n>\\d+)$", (request, response) -> {
					...
					IntStream.range(0, Integer.parseInt(request.getPathParam("n"))).forEach(i -> fibs.add(fibN(i)));
					...
					BlockingLoop.submits(() -> {
						...
						response.setBody(result + request.getHeaders());
						response.send();
					}, throwable -> {
